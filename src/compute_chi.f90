  ...
  do ikx=0, nkx-1
    ...
    do iky=0, nky-1
      ...
      do ikz=0, nkz-1
        ...
        !
        call calc_partial_chi_corr_trace(chi_part, chi0_part, nnu, hk, hkq, ek, ekq, occ_k, occ_kq, Skq)
        !
        chi(:)=chi(:)+chi_part(:)
        chi0(:)=chi0(:)+chi0_part(:)
        !
...
SUBROUTINE calc_partial_chi_corr_trace(chi, chi0, nnu, hk, hkq, eig_k, eig_kq, occ_k, occ_kq, Skq)
  ...
  !
  use constants, only : dp, eps9, cmplx_0, twopi, cmplx_i
  use wanndata,  only : norb
